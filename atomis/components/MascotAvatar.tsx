...
// Custom Float replacement using useFrame
const CustomFloat: React.FC<{
  speed?: number;
  rotationIntensity?: number;
  floatIntensity?: number;
  children: React.ReactNode;
}> = ({ speed = 2, rotationIntensity = 0.2, floatIntensity = 0.5, children }) => {
  ...
  useFrame((state) => {
    ...
    ref.current.rotation.x = Math.sin(t * 0.5) * rotationIntensity * 0.1;
    ref.current.rotation.z = Math.cos(t * 0.3) * rotationIntensity * 0.1;
  });
...
const MascotModel: React.FC<MascotModelProps> = ({ trackingData }) => {
  ...
  return (
    <CustomFloat speed={2} rotationIntensity={0.2} floatIntensity={0.5}>
      <group scale={1.2}>
        {/* --- HEAD GROUP (Rotates) --- */}
        <group ref={headGroupRef}>
            {/* HEAD (Main Sphere) */}
            <mesh args={undefined} position={[0, 0, 0]}>
            ...
            <mesh position={[0, 0.1, 0.2]} scale={[1, 0.8, 1]}>
